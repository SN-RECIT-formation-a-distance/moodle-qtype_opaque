 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_webwork_opaque_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        $mform->addElement('select', 'engineid', get_string('questionengine', 'qtype_webwork_opaque'),
                qtype_webwork_opaque_engine_manager::get()->choices());
        $mform->setType('engineid', PARAM_INT);
        $mform->addRule('engineid', null, 'required', null, 'client');
        $mform->addHelpButton('engineid', 'questionengine', 'qtype_webwork_opaque');

        $mform->addElement('text', 'remoteid',
                get_string('questionid', 'qtype_webwork_opaque'), array('size' => 50));
        $mform->setType('remoteid', PARAM_RAW);
        $mform->addRule('remoteid', null, 'required', null, 'client');
        $mform->addHelpButton('remoteid', 'questionid', 'qtype_webwork_opaque');


        $mform->addElement('text', 'remoteversion',
                get_string('questionversion', 'qtype_webwork_opaque'), array('size' => 3));
        ...
        $mform->addElement('text', 'showhintafter',
                get_string('questionhint', 'qtype_webwork_opaque'), array('size' => 4));
        ...
        $mform->addHelpButton('showhintafter', 'questionhint', 'qtype_webwork_opaque');


        $mform->addElement('text', 'showsolutionafter',
                get_string('questionsolution', 'qtype_webwork_opaque'), 'size="4"');
        ...
        $mform->addHelpButton('showsolutionafter', 'questionsolution', 'qtype_webwork_opaque');
		
		$mform->addElement('select', 'showsolutionaftertest', get_string('endingquestionsolution', 'qtype_webwork_opaque'), array(1=>'oui',0=>'non'));
		$mform->setDefault('showsolutionaftertest', 0);
		$mform->addRule('showsolutionaftertest', null, 'required', null, 'client');
        $mform->addHelpButton('showsolutionaftertest', 'endingquestionsolution', 'qtype_webwork_opaque');
        
        $mform->addElement('text', 'numattemptlock',
                get_string('maxnumattempt', 'qtype_webwork_opaque'), 'size="4"');
        ...
        $mform->addHelpButton('numattemptlock', 'maxnumattempt', 'qtype_webwork_opaque');
		
		$mform->addElement('select', 'exammode', get_string('modeexam', 'qtype_webwork_opaque'), array(1=>'oui',0=>'non'));
		$mform->setDefault('exammode', 0);
		$mform->addRule('exammode', null, 'required', null, 'client');
        $mform->addHelpButton('exammode', 'modeexam', 'qtype_webwork_opaque');
    ...
    public function validation($data, $files) {
        ...
        if (!preg_match("/^$partregexp(\\.$partregexp)*\$/", $data['remoteid'])) {
            $errors['remoteid'] = get_string('invalidquestionidsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }
        
        if (!preg_match('/^\d+\.\d+$/', $data['remoteversion'])) {
            $errors['remoteversion'] = get_string('invalidquestionversionsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }

        if (!preg_match('/^\d+$/', $data['showhintafter'])) {
            $errors['showhintafter'] = get_string('invalidquestionhintsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }
        if (!preg_match('/^\d+$/', $data['showsolutionafter'])) {
            $errors['showsolutionafter'] = get_string('invalidquestionsolutionsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }
        if (!preg_match('/^\d+$/', $data['showsolutionaftertest'])) {
            $errors['showsolutionaftertest'] = get_string('invalidendingquestionsolutionsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }
        if (!preg_match('/^\d+$/', $data['numattemptlock'])) {
            $errors['numattemptlock'] = get_string('invalidmaxnumattemptsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        }
        if (!preg_match('/^\d+$/', $data['exammode'])) {
            $errors['exammode'] = get_string('invalidmodeexamsyntax', 'qtype_webwork_opaque');
            $remoteidok = false;
        } 
        ...
        if ($remoteidok) {
            try {
                ...
                $errors['remoteid'] = get_string('couldnotgetquestionmetadata', 'qtype_webwork_opaque',
                        s($sf->getMessage()));
            }
        }
